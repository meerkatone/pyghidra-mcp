...
### Legacy Tool Search Proxy

Tool search and JSON-string argument coercion are now built into
`pyghidra-mcp`; new configurations should launch `pyghidra-mcp` directly.
`contrib/pyghidra_search_proxy.py` remains available for compatibility with
existing configurations that explicitly launch the older external proxy.

```bash
pip install 'pyghidra-mcp[search]'

# Legacy configuration only:
python contrib/pyghidra_search_proxy.py --project-path /path/to/project
```

Do not place this proxy in front of the FastMCP 4 server for new deployments;
that would apply tool search twice.
